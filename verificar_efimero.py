...
def _instalar_dobles():
    ...
    # Se espía `_borrar_archivos_y_base` y no `_borrar_molde_entero`: el borrado se partió en dos
    # (catálogo con el candado tomado · archivos y base sin candado, que es lo que tarda) y ésta
    # es la parte que destruye. Espiándola quedan cubiertos los dos caminos, porque el otro la llama.
    S._borrar_archivos_y_base = lambda pid, efimero=False: BORRADOS.append((pid, efimero))


_real_borrar = S._borrar_molde_entero
_real_archivos = S._borrar_archivos_y_base
...
S._borrar_archivos_y_base = _real_archivos
S._guardar_catalogo = lambda c, *a, **k: None
