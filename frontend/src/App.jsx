...
export default function App() {
  ...
  const canvasLayout = React.useMemo(() => {
    if (!etqData?.piezas) return { layout: [], width: 850, height: 400, vb: '0 0 850 400' };

    // Siempre usar las posiciones originales del PDF
    const layout = etqData.piezas.map((p) => ({
      ...p,
      tx: 0,
      ty: 0,
      x_new: p.px,
    ...
    const W = etqData.img_w || 850, H = etqData.img_h || 400;
  ...
  const editablesOverlay = React.useMemo(() => {
    ...
    return (ed.objetos || []).flatMap(o => {
      ...
      return canvasLayout.layout
        ...
        .map(p => ({
          nombre: o.nombre, thumb: o.thumb, svg: o.svg, idx: p.idx, rot: tf.rot, scale: tf.scale, dx: tf.dx, dy: tf.dy,
          fcx: ((bb[0] + bb[2]) / 2 - ax) / aw, fcy: ((bb[1] + bb[3]) / 2 - ay) / ah,  // centro, fracción del diseño
          fw: (bb[2] - bb[0]) / aw, fh: (bb[3] - bb[1]) / ah,                          // tamaño, fracción del diseño
        }));
    });
          ...
          <div className="panel animate-fade" style={['moldes', 'arte', 'planilla', 'generar'].includes(pedidoPaso) ? { display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 } : {}}>
            ...
            <div style={['arte', 'planilla', 'generar'].includes(pedidoPaso) ? { flex: 1, minHeight: 0, display: 'flex', flexDirection: 'column' } : {}}>
              ...
              {editorEditOpen && (() => {
                ...
                const _imgDim = (o, p) => { const mr = o.mesa_rect; const aspecto = (mr && mr[3]) ? mr[2] / mr[3] : (p.pw / p.ph); const imgH = p.ph, imgW = aspecto * imgH; return { imgW, imgH, imgX: p.px + (p.pw - imgW) / 2, imgY: p.py }; };
                ...
                {(() => {
                  ...
                  return (
                    <div style={{ marginTop: 16 }}>
                      ...
                      {/* LIENZO tipo espacio infinito: todas las mesas, una al lado de la otra, wrap al pasar el ancho */}
                              ...
                              <div key={mi} style={{ width: 244, background: 'var(--bg-secondary, #141417)', border: '1px solid var(--border-light)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', boxShadow: '0 6px 20px rgba(0,0,0,0.35)' }}>
                                ...
                                <div style={{ padding: '10px 12px' }}>
                                  ...
                                  <div style={{ fontSize: 11, color: 'var(--text-secondary)', marginTop: 4, lineHeight: 1.5 }}>
                                    Tela {hoja.tela} · {hoja.aprovechamiento}% de aprovechamiento
                                  </div>
                                </div>
                              </div>
                            );
