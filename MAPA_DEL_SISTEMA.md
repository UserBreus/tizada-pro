...
## 10.b OBJETOS EDITABLES — cómo se manejan (verificado 2026-07-20; POR OBJETO 2026-07-22)
...
**UNA CAPA PUEDE TENER VARIOS OBJETOS (desde 2026-07-22).** Cada **trazado pintado de primer nivel**
o cada **XObject `Do`** de la capa es un objeto editable **independiente** (mover/rotar/escalar/
espejar/color por separado). `molde_real.objetos_de_capa(page, capa)` (núcleo `_analizar_capa`)
recorre el content-stream una vez con CTM+frame OCG y los separa por **firma de puntos de
construcción** — relleno+trazo del MISMO trazado = un objeto; figuras CONCÉNTRICAS = objetos
distintos. Cada objeto lleva `obj_id` (hash de la geometría, ESTABLE ante reordenamiento) y su bbox.
- **Identidad (`IDENT`):** capa de 1 objeto → nombre de capa (COMPAT, todo sigue igual);
  capa multi → `"nombre<SEP>obj_id"` (`SEP`=U+001F; `motor_pedido.SEP`==`servidor._EDIT_SEP`).
- **Storage anidado:** `prod["editables"][diseno][variable][capa]["objetos"][obj_id]["transforms"|"color"]`
  (multi) / plano `…[capa]["transforms"|"color"]` (1 objeto, compat). `_editables_cfg`/`_editables_color`
  lo aplanan a `{variable:{IDENT:…}}` (lo que consume el motor); `set_editable`/`set_editable_color`
  reciben el IDENT (o `obj_id`) y ANIDAN. `get_editables` expande una capa multi en N ítems
  (`nombre`=IDENT, `label`="capa (i)", `obj_id`, bbox/pos/recolorable/color por objeto).
- **Editor (front) — VERIFICADO POR OBJETO (2026-07-22).** El editor de `App.jsx` clavea TODO por
  `o.nombre`, que YA ES el IDENT que manda `get_editables` → dos objetos de la MISMA capa tienen
  `nombre` distinto y **no colisionan** (la dedup `_objsUnicos`, `editableSel`, `editorTfs`, el
  guardado por `POST /api/productos/editables` y el color por `editable_color` son todos por IDENT).
  Para MOSTRAR usa `o.label` ("Escudo (1)"/"(2)"), nunca el `nombre` con el ␟. **NO hay que
  reconstruir el IDENT en el front** (duplicaría el `obj_id` y rompería el ida-y-vuelta). El comentario
  del `commit 881a05d` ("la identidad por nombre colisiona") era erróneo: se resolvió al bakear el
  `obj_id` dentro de `nombre`. Verificado end-to-end con el arte REAL de 2 objetos del usuario
  (`Diseño Short.ai`, capa «Editable Escudo»: mesa 3 = 1 obj, mesa 4 = **2 elipses** recolorables):
  `get_editables` devuelve 2 ítems con `obj_id`/`nombre` distintos + `recolorable`; un transform y un
  color puestos en UN objeto **no tocan al otro** (`transforms`/`color` del otro = None) y quedan
  anidados en `[capa][objetos][obj_id]`; la mesa de 1 objeto sigue con `obj_id=None` (compat). Harness
  `scratchpad/verif_ui_obj.py` (ejercita los handlers REALES, catálogo en memoria, pid temporal borrado).
- **Primitivas** (`molde_real.py`): `aislar_objeto` (aísla+recolorea UN objeto), `suprimir_objetos`
  (saca sólo esos; `conservar_marcadores=True` = mantiene BDC/EMC para la `suprimir_capas` posterior
  — un raspado los consume, por eso el ORDEN: primero los objetos parciales, después las capas
  enteras), `capa_admite_color_objeto`, `_reescribir_por_indice`.
- **Motor:** `pagina_arte_solo(mesa,capa,color,obj_id=)` aísla por objeto; `pagina_arte_pieza` saca
  del diseño base SÓLO los objetos redibujados de una capa multi (el resto queda vectorial). Una capa
  de 1 objeto NO toca ninguno de estos caminos → **pixel-idéntica a antes** (verificado 0 px).
